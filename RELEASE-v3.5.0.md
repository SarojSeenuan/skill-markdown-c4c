...
### OSS 化 + Web ZIP 修復 + SKILL.md 自己充足型リライト
...
## ▍ 表紙テンプレート + 締めページ 4 パターン

ドキュメントの 1 ページ目が caption 一行だけで真っ白になる問題と、最終ページが「— END OF DOCUMENT —」一行だけになる問題を、テンプレート化で解消。
...
旧版に残っていた「フッタ = `v1.0 · DRAFT` + `現在 / 総数` 形式」という記述を完全削除。
...
ドキュメントごとに caption 行を書き換えるだけで版表記が変わるため、settings.json を都度編集する必要なし。
