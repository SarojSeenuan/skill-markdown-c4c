...
[![Version](https://img.shields.io/badge/version-v5.1.1-8A248C?style=flat-square&labelColor=040104)](./CHANGELOG.md)
...
[![Claude Web](https://img.shields.io/badge/Claude_Web-distributable-040104?style=flat-square&labelColor=888888)](./SETUP-WEB.md)
...
- **Claude Web セットアップガイド `SETUP-WEB.md` / `SETUP-WEB.pdf` を ZIP と docs に同梱** — 18 枚の画面キャプチャ付きで、非開発者でも迷わず登録できる。 配布物がオフライン完結
- **`scripts/install.js` 新規追加** — Node.js 純正クロス OS インストーラ。 `npx -y . markdown-c4c-skill` で Windows / macOS / Linux すべてに対応
- **`build-zip` スクリプトの PDF 除外ルール緩和** — `template-document.pdf` のみ除外、SETUP-WEB.pdf などの説明書 PDF は ZIP に同梱される
- **配布物の整理** — 旧 `markdown-c4c-web-v1.0.0/` フォルダ廃止（ZIP に統合）、旧 ZIP 削除、参照リンク全更新
- **SKILL.md tags 拡張** — `setup-guide` / `claude-web` を追加、発見性向上

詳しくは [`CHANGELOG.md`](./CHANGELOG.md) と [`RELEASE-v5.1.1.md`](./RELEASE-v5.1.1.md) を参照。

> **v5.1.0** の主な改善: 判断ガイド集として reference.md 全面再設計・SKILL.md セクション 3.0 を 5 フェーズ Q&A ワークフローに拡張・ロゴ画像配置案内ルール新設・コンポーネント見出し日本語化（→ [`RELEASE-v5.1.0.md`](./RELEASE-v5.1.0.md)）
...
無料版でも使える配布 ZIP <code>markdown-c4c-v5.1.1.zip</code> を同梱。 社内へそのまま配布可能。
</td>
<td width="33%">
...
[`markdown-c4c-v5.1.1.zip`](./markdown-c4c-v5.1.1.zip) をリポジトリから直接 DL（Download raw file）、または [Releases](https://github.com/SarojSeenuan/skill-markdown-c4c/releases) からアセット取得。
...
> 詳しい手順は **[`SETUP-WEB.md`](./SETUP-WEB.md)**（Windows / macOS / Linux 対応・画面キャプチャ付き）を参照。
...
#### C. `npx` 経由（Node.js 18+ · クロス OS）

本パッケージは `bin: markdown-c4c-skill` を提供。 Node.js があれば Bash も PowerShell も不要で動く。

```bash
# ローカルチェックアウトから直接実行
git clone https://github.com/SarojSeenuan/skill-markdown-c4c.git
cd skill-markdown-c4c
npx -y . markdown-c4c-skill

# または npm link 後にコマンド単体で
npm link
markdown-c4c-skill
```

npm publish 後は `npx -y skill-markdown-c4c` 一発でインストール可能。 Node 純正の `fs.cpSync` のみ使うため、Windows / macOS / Linux で同じ挙動。

#### C-alt. `npx skills`（[npm `skills` パッケージ](https://www.npmjs.com/package/skills) 経由 · 実験的）

```bash
npx skills add SarojSeenuan/skill-markdown-c4c           # プロジェクトローカル
npx skills add SarojSeenuan/skill-markdown-c4c --global  # グローバル
```

> `npx skills` は通常 `skills/<name>/SKILL.md` または root の `SKILL.md` を検出する仕様。 本リポジトリは `skill/`（単数形）配下に置いているため、検出に失敗した場合は **A. インストールスクリプト** または **C. npx 経由** に切り替えてください。

#### D. ZIP 配布から手動展開（Claude Web 用）

リポジトリ直下にある [`markdown-c4c-v5.1.1.zip`](./markdown-c4c-v5.1.1.zip) を Claude Web のサイドバー → カスタマイズ → スキルにそのままアップロード。 自分で再ビルドする場合は `scripts/build-zip.ps1`（Windows）または `scripts/build-zip.sh`（macOS / Linux）を実行。
...
以下のいずれかで `markdown-c4c-v5.1.1.zip` を入手:

- **GitHub から直接ダウンロード**（最も簡単 · コマンド不要）
  - リポジトリトップで `markdown-c4c-v5.1.1.zip` をクリック → 右上の **「Download raw file」** ボタン
  - または直接 URL: `https://github.com/SarojSeenuan/skill-markdown-c4c/raw/main/markdown-c4c-v5.1.1.zip`
- **GitHub Releases から**: [Releases](https://github.com/SarojSeenuan/skill-markdown-c4c/releases) ページの v5.1.1 リリースのアセット欄
...
3. **スキルを追加** ボタンを押す
4. 受け取った `markdown-c4c-v5.1.1.zip` をそのままドラッグ＆ドロップ（解凍不要）
5. 登録完了。 以降の会話で「Markdown で書いて」「提案書まとめて」と頼むと自動で発動

#### ステップ 3: VS Code で PDF 化（任意）

PDF にしたい場合は [VS Code](https://code.visualstudio.com/) と `Markdown PDF` 拡張を入れる。 詳しい手順は画面キャプチャ付きの [`SETUP-WEB.md`](./SETUP-WEB.md) を参照。

#### 配布物の中身

```
markdown-c4c-v5.1.1.zip
└── markdown-c4c/
...
> Claude Web 配布版のセットアップガイドは **[`SETUP-WEB.md`](./SETUP-WEB.md)** に集約されている。
...
│
├── markdown-c4c-v5.1.1.zip                ▍ Claude Web 用配布 ZIP（最新版）
├── SETUP-WEB.md                           ▍ Claude Web セットアップガイド
├── SETUP-WEB.pdf                          ▍ 同 PDF 版
│
├── scripts/                               ▍ インストーラー
│   ├── install.sh                            · macOS / Linux（Bash）
│   ├── install.ps1                           · Windows PowerShell
│   ├── install.js                            · Node.js（クロス OS · npx 対応）
│   ├── build-zip.sh / build-zip.ps1          · Claude Web 互換 ZIP 生成
│   └── validate-zip.py                       · ZIP 構造検証
│
├── docs/                                  ▍ ドキュメント
...
<tr><td><code>v5.1.1</code></td><td>Claude Web セットアップガイド `SETUP-WEB.md`/`pdf` を ZIP・docs・skill 全てに同梱 · `scripts/install.js`（Node.js クロス OS インストーラ）追加 · npx 経由インストール対応 · `build-zip` PDF 除外ルール緩和 · 旧 `markdown-c4c-web-v1.0.0/` 廃止</td></tr>
