...
これは `~/.claude/styles/markdown-pdf.css` (Swiss Modern v3.5) で PDF 化される前提のフォーマット。
...
### 個人名で出したい場合
プロンプトで「個人名で」「○○ 名義で」と明示された場合のみ、settings.json の `class='header-org'` のテキスト部分を指定された名前に書き換える差分を提示する。
変更コードを出すだけで、ユーザーが手動で settings.json を書き換える。

### バージョン表記
**フッタ左は空のまま運用**（v3.5 以降のデフォルト）。 バージョンは Markdown 本文の caption 行で動的指定する:

```markdown
<div class="caption">Author: ○○ ○○　·　Version: v2.3 · FINAL　·　Date: 2026-05-22</div>
```

ドキュメントごとに caption 行を書き換えるだけで版表記が変わる。 settings.json の `<span class='footer-version'>` は空文字で固定する。
...
**★お気に入りパターン**: `<div class="shape-box" data-title="VISUAL ASSET">` で画像 + 説明テキストを組み合わせるパターン。 重要なビジュアルは必ずこの形式で出す。
...
- **`<div class="shape-box" data-title="...">` + 画像 + 説明** = ★お気に入りパターン。 **重要なビジュアルは必ずこの形式**
...
- スタイル定義: `~/.claude/styles/markdown-pdf.css` (Windows: `%APPDATA%\Code\User\...`)
- PDF 出力設定: VS Code settings.json の `markdown-pdf.*` 設定群
